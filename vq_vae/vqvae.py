...
class ResidualStack(nn.Module):
    def __init__(self, num_hiddens, num_residual_layers, num_residual_hiddens, conv_layer: nn.Module = nn.Conv1d):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(
        ...
        for downsampling_layer in range(num_downsampling_layers):
            ...
            curr_hid_dim = out_channels
            conv.add_module(f"relu{downsampling_layer}", nn.ReLU())
        conv.add_module(
            "final_conv",
            conv_layer(
                in_channels=curr_hid_dim,
                out_channels=num_hiddens,
                kernel_size=3,
                padding=1,
            ),
        ...
        # self.activations = CustomLastLayerActivations(
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.ndim == 4:
            ...
            quantized_x = quantized_x.permute(0, 2, 1)
...
class VQVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        (z_quantized, dictionary_loss, commitment_loss, _) = self.quantize(encoded_x)
        x_recon = self.decoder(z_quantized.transpose(1, 2))
        return {
