...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        if args.gpu is not None:
            ...
            networks = [x.to(f"{device}:{args.gpu}") for x in networks]
            ...
            networks = [x.to(device) for x in networks]
        ...
        networks = [torch.nn.DataParallel(x).to(device) for x in networks]
    ...
    if args.distributed:
        ...
        # g_opt = torch.optim.SGD(generator.sub_generators[0].parameters(), 5e-4)
