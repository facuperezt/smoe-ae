...
from utils import plot_kernels_chol, plot_kernel_centers
...
i = -1
orig, recon, mu, z = model.img2block(y[0])[i], model.img2block(y[1])[i], _mu[i], _z[i]
...
axs[1][0].imshow(mu_img.detach().cpu().numpy().squeeze(), cmap='gray', vmin=0, vmax=1)
